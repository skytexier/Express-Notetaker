...
class Helper {
  ...
  deleteNote(id) {
    //Get all notes
    return this.getNote()
    //Then out of notes filter through id's
    .then((notes) => {
      //Setting a const for our note to be deleted
      const deletedNote = id;
      //Looping through the notes to find the matching id
      for (let i = 0; i < notes.length; i++) {
        if (deletedNote === notes[i].id) {
          //Splicing out the note if it exists
          notes.splice(i, 1)
          //Returning a new written db with our new notes array
